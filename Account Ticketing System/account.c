...
void getUserLogin(struct UserLogin* userLogin)
{
    ...
    printf("----------------------------------------\n");
    while (1)
    { // -- loop to validate the user login name input by the user
    ...
    while (1)
    {
        ...
        {
            // password input is strong ; password validation is success//printf("\n---- %d %d %d %d ----- \n", digitCounter , uppercaseCounter , lowercaseCounter , symbolCounter);
            //int i =0;
...
void updateAccount(struct Account* account)
{
    ...
    if (choice == 1)
    {
        ...
        if (c == 'A')
        { //agent type requires user login information
